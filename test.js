...
class GridWorm {
    ...
    getColor(opacity, isRandom = true, index = 0) {
        ...
        var colors =
            [
                col('fe8019', opacity), col('fb4934', opacity)
            ];
        if (isRandom) {
    ...
    */
    ...
    update(deltaTime) {
        ...
        if (latestJunction !== currentCoord) {
            ...
            while(this.isInMemory(this.getJunctionReached({x: this.xCoord + newVelocity.x / this.speed, y: this.yCoord + newVelocity.y / this.speed})))
                newVelocity = this.getVelocity();
